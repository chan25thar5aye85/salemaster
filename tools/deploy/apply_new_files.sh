...
# apply_new_files.sh
#
# Creates brand-new files for the PurchaseFinalizer + TimeFilter-on-P&L work.
# Refuses to overwrite existing files unless --force is passed.
#
# Usage:
#   ./tools/deploy/apply_new_files.sh           # fail if target exists
#   ./tools/deploy/apply_new_files.sh --force   # overwrite
# ─────────────────────────────────────────────────────────────────────────────
set -euo pipefail
...
# Resolve repo root (script lives in <repo>/tools/deploy)
REPO_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
cd "$REPO_ROOT"

PKG_ROOT="app/src/main/java/com/akari/retailer"
...
write_file () {
    ...
    if [[ -e "$target" && $FORCE -eq 0 ]]; then
        echo "⚠  SKIP (exists): $target   (pass --force to overwrite)"
        return 0
    fi
    cat > "$target"
    echo "✅ wrote $target"
...
echo "▸ Writing new files under $REPO_ROOT"
echo

# ── 1. PurchaseFinalizer interface ──────────────────────────────────────────
write_file "$PKG_ROOT/features/inventory/data/repository/PurchaseFinalizer.kt" <<'KOTLIN'
...
# ── 2. FirestorePurchaseFinalizer impl ──────────────────────────────────────
write_file "$PKG_ROOT/features/inventory/data/remote/FirestorePurchaseFinalizer.kt" <<'KOTLIN'
...
echo
echo "Next steps:"
echo "  1. Run tools/deploy/patch_existing.py  (patches existing files)"
echo "  2. Rebuild:  ./gradlew :app:assembleDebug"
